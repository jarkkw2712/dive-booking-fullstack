...
test("booking creator comes from authenticated server user and remains immutable",()=>{
  ...
  assert.match(sql,/created_by=coalesce\(created_by/);assert.match(sql,/list_bookings_json_v10/);assert.match(route,/createdBy:req\.user\.username/);assert.match(route,/upsert_booking_v18/);
});
