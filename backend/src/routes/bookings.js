...
router.get("/",async(req,res)=>{const {data,error}=await bookingRpc("list_bookings_json_v18","list_bookings_json_v17");if(error)return res.status(500).json({error:error.message});res.json(data||[])});
...
router.post("/",requirePermission("createBooking"),async(req,res)=>{const {data,error}=await bookingRpc("upsert_booking_v18","upsert_booking_v17",{p_booking:{...req.body,createdBy:req.user.username}});if(error)return res.status(500).json({error:error.message});res.json(data)});
router.put("/:code",requirePermission("editBooking"),async(req,res)=>{const {data,error}=await bookingRpc("upsert_booking_v18","upsert_booking_v17",{p_booking:{...req.body,bookingCode:req.params.code,createdBy:req.user.username}});if(error)return res.status(500).json({error:error.message});res.json(data)});
