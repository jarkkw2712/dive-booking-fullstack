...
router.get("/",async(req,res)=>{const {data,error}=await supabaseAdmin.rpc("list_bookings_json_v4");if(error)return res.status(500).json({error:error.message});res.json(data||[])});
router.post("/check-duplicate",async(req,res)=>{
  const booking=req.body||{};
  const {data,error}=await supabaseAdmin.rpc("list_bookings_json_v4");
...
router.post("/",requirePermission("createBooking"),async(req,res)=>{const {data,error}=await supabaseAdmin.rpc("upsert_booking_v4",{p_booking:req.body});if(error)return res.status(500).json({error:error.message});res.json(data)});
router.put("/:code",requirePermission("editBooking"),async(req,res)=>{const {data,error}=await supabaseAdmin.rpc("upsert_booking_v4",{p_booking:{...req.body,bookingCode:req.params.code}});if(error)return res.status(500).json({error:error.message});res.json(data)});
