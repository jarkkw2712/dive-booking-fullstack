...
router.get("/",async(req,res)=>{const {data,error}=await bookingRpc("list_bookings_json_v7","list_bookings_json_v6");if(error)return res.status(500).json({error:error.message});res.json(data||[])});
router.post("/check-duplicate",async(req,res)=>{
  const booking=req.body||{};
  const {data,error}=await bookingRpc("list_bookings_json_v7","list_bookings_json_v6");
...
router.post("/",requirePermission("createBooking"),async(req,res)=>{const {data,error}=await bookingRpc("upsert_booking_v8","upsert_booking_v7",{p_booking:req.body});if(error)return res.status(500).json({error:error.message});res.json(data)});
router.put("/:code",requirePermission("editBooking"),async(req,res)=>{const {data,error}=await bookingRpc("upsert_booking_v8","upsert_booking_v7",{p_booking:{...req.body,bookingCode:req.params.code}});if(error)return res.status(500).json({error:error.message});res.json(data)});
