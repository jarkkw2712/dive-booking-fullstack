...
const accommodationQuantity=person=>person?.accommodationId?Math.max(Number(person.accommodationQty??1),0):0;
const accommodationUnitPrice=person=>Math.max(money(person?.accommodationPrice),0);
const accommodationRevenue=person=>person?.accommodationBookedBy==="company"?accommodationQuantity(person)*accommodationUnitPrice(person):0;
...
function accommodationSummary(entries){
  ...
  return{
    parkHouse:people.filter(person=>person.accommodationId==="park_house").reduce((sum,person)=>sum+accommodationQuantity(person),0),
    parkTent:people.filter(person=>person.accommodationId==="park_tent").reduce((sum,person)=>sum+accommodationQuantity(person),0),
    none:people.filter(person=>!person.accommodationId).length,
    customerSelfBooked:people.filter(person=>person.accommodationId&&person.accommodationBookedBy!=="company").reduce((sum,person)=>sum+accommodationQuantity(person),0),
    companyBooked:people.filter(person=>person.accommodationId&&person.accommodationBookedBy==="company").reduce((sum,person)=>sum+accommodationQuantity(person),0),
    customerReferenceValue:people.filter(person=>person.accommodationBookedBy!=="company").reduce((sum,person)=>sum+accommodationQuantity(person)*accommodationUnitPrice(person),0),
    companyRevenue:people.reduce((sum,person)=>sum+accommodationRevenue(person),0),
    note:"ลูกค้าจองเองเก็บเพื่อการปฏิบัติงานและไม่รวมรายได้; เราจองให้รวมจำนวน × ราคาในยอด Booking"
  };
}
...
function passengerRows(entries,{includeHealth=false}={}){
  return entries.flatMap(({booking,direction,date})=>passengersOf(booking).map(person=>({
    ...
    accommodationBookedBy:person.accommodationId?(person.accommodationBookedBy==="company"?"เราจองให้":"ลูกค้าจองเอง"):"",
    accommodationQty:accommodationQuantity(person),
    accommodationPrice:accommodationUnitPrice(person),
    accommodationTotal:accommodationRevenue(person),
    accommodationReference:person.parkAccommodationReference||"",
    accommodationArrangement:person.accommodationId?(person.accommodationBookedBy==="company"?"เราจองให้":"ลูกค้าจองเอง"):"",
...
function serviceRevenueLines(booking,paymentMethods){
  ...
  for(const person of passengersOf(booking)){
    ...
    if(person.accommodationId&&person.accommodationBookedBy==="company")rows.push({source:"accommodation",amount:accommodationRevenue(person),paymentMethod:booking.paymentMethod||defaultPayment(paymentMethods,"general")});
  }
...
function tourExpenseReferenceReport(bookings,date,toDate,paymentMethods){
  ...
  const monthly=tourMonthlyReferenceReport(bookings,date,toDate,paymentMethods);
  return{date,type:"tour_expense_reference",title:"รายงานทัวร์และค่าใช้จ่าย",purpose:"ยอดตามวันเดินทางไป แยกหมวดรายได้และช่องทางชำระเงิน",range:{from:date,to:toDate},rows,totals,monthlyRows:monthly.rows,monthlyTotals:monthly.totals,accountNames:namedPaymentAccounts,sourceNotes:["วันที่อ้างอิงวันเดินทางไปของ Booking","ตั๋วเรือใช้ Program code boat_ticket; Program อื่นเป็นแพ็คเกจ","เต็นท์ใช้อุปกรณ์ code tent","ที่พักรวมเป็นรายได้เฉพาะรายการที่เลือก เราจองให้"]};
}
...
function vanDailyReferenceReport(bookings,date,toDate,paymentMethods){
  ...
  return{date,type:"van_daily_reference",title:"รายงานรถตู้ (เฉพาะรถตู้)",purpose:"ยอดค่ารถตู้ตามวันเดินทางไป แยกเงินสดและเงินโอน",range:{from:date,to:toDate},rows,totals,monthlyRows:monthly.rows,monthlyTotals:monthly.totals,sourceNotes:["รวมเฉพาะวิธีการเดินทางรถตู้","ยอดขาไปและขากลับใช้ช่องทางชำระของแต่ละขา","เงินสด/โอนพิจารณาจาก code ใน Payment Method Master"]};
}
...
function agentReferenceReport(bookings,date,toDate,masterAgents=[]){
  const agentOrder=new Map((masterAgents||[]).map((agent,index)=>[String(agent.agent_name||"").trim(),Number(agent.sort_order??index)])),groups=new Map();
  const selected=bookings.filter(booking=>activeBooking(booking)&&booking.travelDate>=date&&booking.travelDate<=toDate);
  for(const booking of selected){
    const agent=String(booking.agentName||"").trim()||"ไม่ระบุ Agent",people=passengersOf(booking),counts=passengerCounts(booking);
    let boatAmount=0,packageAmount=0;
    for(const person of people){if(!person.program)continue;const amount=lineAmount(person.program.qty,person.program.price);if(isBoatProgram(person.program))boatAmount+=amount;else packageAmount+=amount}
    const group=groups.get(agent)||{agent,rows:[]};
    group.rows.push({date:booking.travelDate||"",leader:leaderName(booking),adult:counts.adult,child:counts.child,infant:counts.infant,foc:counts.foc,boatAmount,packageAmount,total:boatAmount+packageAmount,paymentDate:"",bookingCode:booking.bookingCode||""});
    groups.set(agent,group);
  }
  const totalFields=["adult","child","infant","foc","boatAmount","packageAmount","total"];
  const pages=[...groups.values()].sort((a,b)=>(agentOrder.get(a.agent)??Number.MAX_SAFE_INTEGER)-(agentOrder.get(b.agent)??Number.MAX_SAFE_INTEGER)||a.agent.localeCompare(b.agent,"th")).map(group=>{
    group.rows.sort((a,b)=>a.date.localeCompare(b.date)||a.bookingCode.localeCompare(b.bookingCode));
    return{...group,totals:Object.fromEntries(totalFields.map(field=>[field,group.rows.reduce((sum,row)=>sum+money(row[field]),0)]))};
  });
  return{date,type:"agent_reference",title:"รายงานเอเย่น",purpose:"สรุปยอดโปรแกรมแยกตาม Agent และวันเดินทางไป",range:{from:date,to:toDate},pages,summary:{agents:pages.length,bookings:selected.length,total:pages.reduce((sum,page)=>sum+page.totals.total,0)}};
}

const transportationMethodCode=(value,methods=[])=>{const key=normalized(value),master=(methods||[]).find(item=>normalized(item.method_id)===key||normalized(item.method_name)===key);return normalized(master?.method_id||value)};
const transportationBucket=(value,methods=[])=>{const code=transportationMethodCode(value,methods);if(code==="lignite")return"lignite";if(code==="bks")return"bks";if(code==="private_car")return"privateCar";if(code.includes("van")||code.includes("pv"))return"van";return"other"};
function customerTravelDailyReferenceReport(bookings,date,toDate,transportationMethods=[]){
  const pages=reportDays(date,toDate).map(day=>{
    const groups=new Map();
    for(const booking of bookings.filter(activeBooking))for(const person of passengersOf(booking)){
      const returnDate=person.passengerReturnDate||booking.returnDate||"";if(returnDate!==day)continue;
      const key=booking.bookingCode||leaderName(booking),row=groups.get(key)||{bookingCode:booking.bookingCode||"",leader:leaderName(booking),arrivalDates:new Set(),pax:0,lignite:0,bks:0,privateCar:0,van:0,other:0,otherMethods:new Map(),bookingNote:booking.bookingNote||""};
      const arrivalDate=person.passengerTravelDate||booking.travelDate||"";if(arrivalDate)row.arrivalDates.add(arrivalDate);row.pax++;
      const method=person.returnTransportationMethod||person.transportationMethod||booking.transportationMethod||"ไม่ระบุ",bucket=transportationBucket(method,transportationMethods);row[bucket]++;
      if(bucket==="other")row.otherMethods.set(method,(row.otherMethods.get(method)||0)+1);groups.set(key,row);
    }
    const rows=[...groups.values()].sort((a,b)=>[...a.arrivalDates].join(",").localeCompare([...b.arrivalDates].join(","))||a.bookingCode.localeCompare(b.bookingCode)).map((row,index)=>{const other=[...row.otherMethods].map(([name,count])=>`${name} ${count} คน`).join(", "),note=[row.bookingNote,other?`วิธีเดินทางอื่น: ${other}`:""].filter(Boolean).join(" · ");return{no:index+1,arrivalDate:[...row.arrivalDates].sort().join(", "),leader:row.leader,pax:row.pax,lignite:row.lignite,bks:row.bks,privateCar:row.privateCar,van:row.van,note}});
    const totals={pax:rows.reduce((sum,row)=>sum+row.pax,0),lignite:rows.reduce((sum,row)=>sum+row.lignite,0),bks:rows.reduce((sum,row)=>sum+row.bks,0),privateCar:rows.reduce((sum,row)=>sum+row.privateCar,0),van:rows.reduce((sum,row)=>sum+row.van,0)};
    return{date:day,rows,totals};
  });
  return{date,type:"customer_travel_daily_reference",title:"ตารางเดินทางลูกค้ารายวัน",purpose:"รายชื่อลูกค้าที่เดินทางกลับ แยกจำนวนตามวิธีการเดินทาง",range:{from:date,to:toDate},pages,summary:{days:pages.length,pax:pages.reduce((sum,page)=>sum+page.totals.pax,0)}};
}
...
function tourMonthlyReferenceReport(bookings,date,toDate,paymentMethods){
  const rows=reportMonths(date,toDate).map(month=>{const selected=bookings.filter(booking=>activeBooking(booking)&&String(booking.travelDate||"").slice(0,7)===month),services=selected.flatMap(booking=>serviceRevenueLines(booking,paymentMethods)),deposits=selected.map(booking=>({amount:money(booking.depositAmount),paymentMethod:booking.depositPaymentMethod||booking.paymentMethod||""})),accountLines=[...services,...deposits],cash=services.filter(line=>paymentKind(line.paymentMethod,paymentMethods)==="cash").reduce((sum,line)=>sum+line.amount,0),transfer=services.filter(line=>paymentKind(line.paymentMethod,paymentMethods)==="transfer").reduce((sum,line)=>sum+line.amount,0);return{month,cash,transfer,deposit:deposits.reduce((sum,line)=>sum+line.amount,0),credit:selected.reduce((sum,booking)=>sum+money(booking.creditAmount),0),rungruedeeTransfer:accountLines.filter(line=>namedAccount(line.paymentMethod)==="รุ่งฤดี"&&paymentKind(line.paymentMethod,paymentMethods)==="transfer").reduce((sum,line)=>sum+line.amount,0)}});
  const fields=["cash","transfer","deposit","credit","rungruedeeTransfer"],totals=Object.fromEntries(fields.map(field=>[field,rows.reduce((sum,row)=>sum+money(row[field]),0)]));
  return{date,type:"tour_monthly_reference",title:"รายงานสรุปรายการทัวร์",purpose:"สรุปรายเดือนตามวันเดินทางไป",range:{from:date,to:toDate},rows,totals,sourceNotes:["มัดจำและขายเชื่อเป็นข้อมูลประกอบและไม่บวกซ้ำกับยอดสด/โอน","ตัดคอลัมน์เบิกทัวร์และคงเหลือตามข้อกำหนด"]};
}
function vanMonthlyReferenceReport(bookings,date,toDate,paymentMethods){
  ...
  const totals={cash:rows.reduce((sum,row)=>sum+row.cash,0),transfer:rows.reduce((sum,row)=>sum+row.transfer,0),total:rows.reduce((sum,row)=>sum+row.total,0)};
  return{date,type:"van_monthly_reference",title:"รายงานสรุปรายการรถตู้",purpose:"สรุปค่ารถตู้รายเดือนตามวันเดินทางไป",range:{from:date,to:toDate},rows,totals,sourceNotes:["รวมเฉพาะวิธีการเดินทางรถตู้","เงินสด/โอนพิจารณาจาก code ใน Payment Method Master"]};
}
...
function managementReport(bookings,financialRows,expenseRows,date,toDate=date,masterAddOns=[],masterAccommodations=[]){
  ...
  const today=rows[0]||{};
  const categoryNames=["ค่าตั๋วเรือ","ที่พัก","มัดจำ","ขายเชื่อ","ค่าน้ำ","ค่าน้ำเกาะ","หน้ากาก","ชูชีพ","ฟิน","น้ำแข็ง","เต็นท์","เหมาเรือ","ค่าระวาง","อื่นๆ","รวมรายได้"];
  const incomeMatrix=categoryNames.map(category=>({category,values:Object.fromEntries(days.map(day=>{const daily=bookings.filter(booking=>activeBooking(booking)&&booking.travelDate===day);let amount=0;if(category==="ค่าตั๋วเรือ")amount=daily.flatMap(passengersOf).reduce((sum,p)=>sum+money(p.program?.qty||1)*money(p.program?.price),0);else if(category==="ที่พัก")amount=daily.flatMap(passengersOf).reduce((sum,p)=>sum+accommodationRevenue(p),0);else if(category==="มัดจำ")amount=daily.reduce((sum,b)=>sum+money(b.depositAmount),0);else if(category==="ขายเชื่อ")amount=daily.reduce((sum,b)=>sum+money(b.creditAmount),0);else if(category==="รวมรายได้")amount=daily.reduce((sum,b)=>sum+money(b.totalAmount),0);else amount=daily.flatMap(passengersOf).flatMap(p=>[...(p.preAddOns||[]).filter(a=>a.selected),...(p.islandAddOns||[])]).filter(a=>incomeCategory(a.name)===category).reduce((sum,a)=>sum+money(a.qty||1)*money(a.price),0);return[day,amount]}))}));
  ...
  const arrivalPeople=arrivalEntries.flatMap(({booking})=>passengersOf(booking)),accommodationItems=(masterAccommodations||[]).map(item=>{const selected=arrivalPeople.filter(person=>person.accommodationId===item.accommodation_id);return{code:item.accommodation_id,name:item.accommodation_name,qty:selected.reduce((sum,person)=>sum+accommodationQuantity(person),0),defaultPrice:money(item.default_price),amount:selected.reduce((sum,person)=>sum+accommodationRevenue(person),0)}});
  return{
...
const accommodationText=person=>person.accommodationId?`${person.accommodationName||person.accommodationId} x ${accommodationQuantity(person)} (${person.accommodationBookedBy==="company"?"เราจองให้":"ลูกค้าจองเอง"})`:"";
...
export function buildPrintCenterReport({bookings=[],financialRows=[],expenseRows=[],paymentMethods=[],masterAddOns=[],masterAccommodations=[],masterAgents=[],transportationMethods=[],date,toDate,type}){
  ...
  if(type==="customer_travel_daily_reference")return customerTravelDailyReferenceReport(active,date,toDate,transportationMethods);
  ...
  if(type==="counter"){
    ...
    rows=entries.map(({booking,direction,date})=>({
      ...
      accommodation:uniqueValues(passengersOf(booking),accommodationText),
