...
test("frontend assets are cache-busted and expose a visible deployment version",()=>{
  ...
  assert.match(html,/Version 2026\.09\.04-6/);
  for(const asset of ["css/style.css","js/api.js","js/smartPaste.js","js/csvImport.js","js/app.js","js/financial.js"])assert.match(html,new RegExp(`${asset.replace(/[/.]/g,"\\$&")}\\?v=20260904-6`));
});
...
test("CEO report is paginated into structured A4 landscape sections",()=>{
  ...
  assert.match(app,/category:`รายจ่าย: \$\{row\.category\}`/);assert.match(app,/\.\.\.expenses,expenseTotal,net/);assert.match(app,/matrixDates\.length<=7/);
});
