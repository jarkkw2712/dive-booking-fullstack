...
async function createInitialInvoice(){
  ...
  const items=[];(booking.passengers||[]).forEach(person=>{const name=[person.title,person.firstName,person.lastName].filter(Boolean).join(" ");if(person.program)items.push({sourceType:"program",passengerName:name,itemCode:person.program.programId,itemName:person.program.name,qty:Number(person.program.qty||1),unitPrice:Number(person.program.price||0),discountAmount:0,description:""});if(person.accommodationId&&person.accommodationBookedBy==="company"&&Number(person.accommodationQty||0)>0)items.push({sourceType:"accommodation",passengerName:name,itemCode:person.accommodationId,itemName:`ที่พัก - ${person.accommodationName||person.accommodationId}`,qty:Number(person.accommodationQty||0),unitPrice:Number(person.accommodationPrice||0),discountAmount:0,description:"เราจองให้"});if(Number(person.transportationAmount||0)>0)items.push({sourceType:"transport_outbound",passengerName:name,itemCode:"transport_outbound",itemName:`ค่าเดินทางไป - ${person.transportationMethod||"ไม่ระบุ"}`,qty:1,unitPrice:Number(person.transportationAmount||0)});if(Number(person.returnTransportationAmount||0)>0)items.push({sourceType:"transport_return",passengerName:name,itemCode:"transport_return",itemName:`ค่าเดินทางกลับ - ${person.returnTransportationMethod||"ไม่ระบุ"}`,qty:1,unitPrice:Number(person.returnTransportationAmount||0)});(person.preAddOns||[]).filter(x=>x.selected).forEach(x=>items.push({sourceType:"pre_addon",passengerName:name,itemCode:x.id,itemName:x.name,qty:Number(x.qty||1),unitPrice:Number(x.price||0)}));(person.islandAddOns||[]).forEach(x=>items.push({sourceType:"island_addon",passengerName:name,itemCode:x.id,itemName:x.name,qty:Number(x.qty||1),unitPrice:Number(x.price||0)}));});
