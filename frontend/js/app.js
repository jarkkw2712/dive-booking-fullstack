...
function createPassenger(i=0){const pr=master.programs?.[0]||{program_id:"boat_ticket",program_name:"ตั๋วเรือ",default_price:1500};const policy=programAccommodationPolicy(pr);return{title:i===0?document.getElementById("leaderTitle").value:"",firstName:i===0?document.getElementById("leaderFirstName").value:"",lastName:i===0?document.getElementById("leaderLastName").value:"",age:"",phone:i===0?document.getElementById("phone").value:"",island:"",foodAllergy:"",medicalNote:"",parkAccommodationType:policy==="none"?"none":"unknown",parkAccommodationBookedBy:policy==="none"?"customer":"unknown",parkAccommodationArrangement:policy==="none"?"not_required":"undecided",parkAccommodationReference:"",parkAccommodationNote:"",tentCreditAmount:0,isLeader:i===0,program:{programId:pr.program_id,name:pr.program_name,qty:1,price:Number(pr.default_price),defaultPrice:Number(pr.default_price),accommodationPolicy:policy,selfBookedTentCredit:Number(pr.self_booked_tent_credit||0),priceReason:"ราคา Default"},preAddOns:(master.addOns||[]).map(a=>({id:a.addon_id,name:a.addon_name,selected:false,qty:1,price:Number(a.default_price),defaultPrice:Number(a.default_price)})),islandAddOns:[]}}
...
function programAccommodationPolicy(program){if(program?.accommodationPolicy)return program.accommodationPolicy;if(program?.accommodation_policy)return program.accommodation_policy;if((program?.programId||program?.program_id)==="one_day")return"none";if(["two_day","three_day"].includes(program?.programId||program?.program_id))return"required";return"optional"}
function applyAccommodationPolicyToPassenger(index,reset=false){const person=passengers[index],program=person?.program?.accommodationPolicy?person.program:master.programs.find(item=>item.program_id===person?.program?.programId)||person?.program||{},policy=programAccommodationPolicy(program);if(!person)return;person.program.accommodationPolicy=policy;person.program.selfBookedTentCredit=Number(program.self_booked_tent_credit??program.selfBookedTentCredit??person.program.selfBookedTentCredit??0);if(policy==="none"){person.parkAccommodationArrangement="not_required";person.parkAccommodationType="none";person.parkAccommodationBookedBy="customer";person.tentCreditAmount=0;return}if(reset||!person.parkAccommodationArrangement){person.parkAccommodationArrangement="undecided";person.parkAccommodationType="unknown";person.parkAccommodationBookedBy="unknown";person.tentCreditAmount=0}else if(person.parkAccommodationArrangement!=="customer_self_booked")person.tentCreditAmount=0;else if(person.tentCreditAmount===undefined||person.tentCreditAmount===null)person.tentCreditAmount=Number(person.program.selfBookedTentCredit||0)}
function setAccommodationArrangement(index,value){const person=passengers[index];person.parkAccommodationArrangement=value;if(value==="not_required"){person.parkAccommodationType="none";person.parkAccommodationBookedBy="customer";person.tentCreditAmount=0}else if(value==="included_tent"){person.parkAccommodationType="park_tent";person.parkAccommodationBookedBy="park";person.tentCreditAmount=0}else if(value==="customer_self_booked"){if(["none","unknown"].includes(person.parkAccommodationType))person.parkAccommodationType="park_house";person.parkAccommodationBookedBy="customer";person.tentCreditAmount=Number(person.program.selfBookedTentCredit||0)}else{person.parkAccommodationType="unknown";person.parkAccommodationBookedBy="unknown";person.tentCreditAmount=0}renderPassengers();refreshSummary()}
function changeProgram(pi,id){const pr=master.programs.find(x=>x.program_id===id);if(!pr)return;passengers[pi].program={programId:pr.program_id,name:pr.program_name,qty:1,price:Number(pr.default_price),defaultPrice:Number(pr.default_price),accommodationPolicy:programAccommodationPolicy(pr),selfBookedTentCredit:Number(pr.self_booked_tent_credit||0),priceReason:"ราคา Default"};applyAccommodationPolicyToPassenger(pi,true);if(id!=="boat_ticket")passengers[pi].preAddOns.forEach(item=>item.selected=false);renderPassengers();refreshSummary();}
function copyLeaderPackage(pi){if(pi===0||!passengers[0])return;passengers[pi].program=structuredClone(passengers[0].program);passengers[pi].preAddOns=structuredClone(passengers[0].preAddOns);applyAccommodationPolicyToPassenger(pi,true);renderPassengers();refreshSummary();}
...
function applyPassengerBusinessRules(){document.querySelectorAll("#passengerList .passenger-card").forEach((card,index)=>{const person=passengers[index];applyAccommodationPolicyToPassenger(index);if(index>0&&!card.querySelector(".copy-leader-package")){const button=document.createElement("button");button.type="button";button.className="btn soft copy-leader-package";button.textContent="คัดลอก Program/Add-on จากหัวหน้าทริป";button.addEventListener("click",()=>copyLeaderPackage(index));card.querySelector("h3")?.after(button)}const typeSelect=card.querySelector('select[onchange*="parkAccommodationType"]');if(typeSelect){if(!typeSelect.querySelector('option[value="unknown"]'))typeSelect.add(new Option("ยังไม่ทราบ","unknown"));typeSelect.value=person.parkAccommodationType||"unknown";typeSelect.disabled=!["customer_self_booked","undecided"].includes(person.parkAccommodationArrangement)}if(!card.querySelector(".accommodation-rule")){const box=document.createElement("div"),policy=person.program.accommodationPolicy,options=policy==="none"?[["not_required","ไม่นอน (อัตโนมัติสำหรับ One Day)"]]:[["undecided",policy==="optional"?"ยังไม่ทราบ / แก้ภายหลัง":"กรุณาเลือกที่พัก"],["included_tent","ใช้เต็นท์ที่รวมในโปรแกรม"],["customer_self_booked","ลูกค้าจองที่พักเอง - คืนค่าเต็นท์"]];box.className="service accommodation-rule";box.innerHTML=`<b>กฎที่พักตามโปรแกรม</b><select ${policy==="none"?"disabled":""} onchange="setAccommodationArrangement(${index},this.value)">${options.map(([value,label])=>`<option value="${value}" ${person.parkAccommodationArrangement===value?"selected":""}>${label}</option>`).join("")}</select><div class="line"><span>เครดิตคืนค่าเต็นท์</span><b>${money(person.tentCreditAmount||0)} บาท</b></div><div class="muted">${policy==="optional"?"วันกลับและที่พักเป็นข้อมูลประมาณการ แก้ไขภายหลังได้":policy==="required"?"โปรแกรมค้างคืนต้องระบุที่พักก่อนบันทึก":"ไม่คิดที่พักสำหรับ One Day"}</div>`;card.querySelector(".form-grid")?.after(box)}const preSection=card.querySelectorAll(".service")[2],locked=person?.program?.programId!=="boat_ticket";if(preSection){preSection.querySelectorAll("input").forEach(input=>input.disabled=locked);preSection.title=locked?"Package รวมบริการ Pre Add-on แล้ว":""}})}
...
function validateBooking(b){if(!b.travelDate)return"กรุณาระบุวันเดินทาง";if(b.tripType==="round_trip"&&!b.returnDate)return"กรุณาระบุวันเดินทางกลับ (สามารถแก้ภายหลังได้)";if(b.returnDate&&b.returnDate<b.travelDate)return"วันเดินทางกลับต้องไม่ก่อนวันเดินทาง";if(!b.leaderFirstName||!b.leaderLastName)return"กรุณาระบุชื่อและนามสกุลหัวหน้าทริป";if(!b.passengers.length||b.passengers.some(p=>!p.firstName||!p.lastName))return"กรุณาระบุชื่อและนามสกุลผู้เดินทางทุกคน";if(b.passengers.some(p=>p.program?.accommodationPolicy==="required"&&p.parkAccommodationArrangement==="undecided"))return"โปรแกรมค้างคืนต้องระบุว่าใช้เต็นท์ในโปรแกรมหรือลูกค้าจองที่พักเอง";return""}
...
function loadBookingToForm(b){editingCode=b.bookingCode;["tripType","travelDate","returnDate","leaderTitle","leaderFirstName","leaderLastName","phone","source","agentName","status","paymentMethod","bookingNote"].forEach(id=>{const el=document.getElementById(id);if(el)el.value=b[id]||""});passengers=b.passengers||[];passengers.forEach((_,index)=>applyAccommodationPolicyToPassenger(index));document.getElementById("paxCount").value=passengers.length||1;toggleReturnDate();renderPassengers();refreshSummary();document.getElementById("saveNewBtn").classList.add("hidden");document.getElementById("saveEditBtn").classList.remove("hidden");}
...
const reportMoneyColumns=new Set(["totalAmount","tentCreditAmount","tentCreditDue","expectedRevenue","actualReceived","outstanding"]);
...
const masterDataConfig={programs:{id:"program_id",name:"program_name",price:"default_price"},addons:{id:"addon_id",name:"addon_name",price:"default_price"},agents:{id:"agent_id",name:"agent_name"},boats:{id:"boat_id",name:"boat_name"},islands:{id:"island_id",name:"island_name"},price_reasons:{id:"reason_id",name:"reason_name"},payment_methods:{id:"method_id",name:"method_name"},statuses:{id:"status_id",name:"status_name"}};
...
function configureMasterDataPriceField(){document.getElementById("mdpPrice").closest("div").classList.toggle("hidden",!masterDataConfig[mdCat].price);document.getElementById("mdpAccommodationPolicyField").classList.toggle("hidden",mdCat!=="programs");document.getElementById("mdpTentCreditField").classList.toggle("hidden",mdCat!=="programs")}
function openMasterDataEditor(){document.getElementById("masterDataEditor").classList.remove("hidden");["mdpEditingId","mdpCode","mdpName","mdpDescription"].forEach(id=>document.getElementById(id).value="");document.getElementById("mdpCode").disabled=false;document.getElementById("mdpPrice").value=0;document.getElementById("mdpAccommodationPolicy").value="optional";document.getElementById("mdpTentCredit").value=0;document.getElementById("mdpSort").value=0;document.getElementById("mdpActive").value="true";configureMasterDataPriceField()}
function editMasterDataProItem(index){const row=mdRows[index],config=masterDataConfig[mdCat];if(!row)return;document.getElementById("masterDataEditor").classList.remove("hidden");document.getElementById("mdpEditingId").value=row[config.id];document.getElementById("mdpCode").value=row[config.id]||"";document.getElementById("mdpCode").disabled=true;document.getElementById("mdpName").value=row[config.name]||"";document.getElementById("mdpPrice").value=config.price?Number(row[config.price]||0):0;document.getElementById("mdpAccommodationPolicy").value=row.accommodation_policy||"optional";document.getElementById("mdpTentCredit").value=Number(row.self_booked_tent_credit||0);document.getElementById("mdpSort").value=Number(row.sort_order||0);document.getElementById("mdpDescription").value=row.description||"";document.getElementById("mdpActive").value=String(row.active_flag!==false);configureMasterDataPriceField();document.getElementById("masterDataEditor").scrollIntoView({behavior:"smooth",block:"start"})}
function closeMasterDataEditor(){document.getElementById("masterDataEditor").classList.add("hidden")}
async function saveMasterDataProItem(){const editingId=val("mdpEditingId"),item={code:val("mdpCode"),name:val("mdpName"),default_price:Number(val("mdpPrice")||0),accommodation_policy:val("mdpAccommodationPolicy"),self_booked_tent_credit:Number(val("mdpTentCredit")||0),sort_order:Number(val("mdpSort")||0),description:val("mdpDescription"),active_flag:val("mdpActive")==="true"};if(!item.code||!item.name)return alert("กรุณากรอก Code และชื่อรายการ");if(editingId)await API.updateMdp(mdCat,editingId,item);else await API.saveMdp(mdCat,item);alert(editingId?"แก้ไขรายการแล้ว":"เพิ่มรายการแล้ว");closeMasterDataEditor();await loadMasterDataPro(mdCat)}
