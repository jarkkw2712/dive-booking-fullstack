...
function showPage(id){document.querySelectorAll(".page").forEach(p=>p.classList.remove("active"));document.getElementById(id)?.classList.add("active");if(id==="dashboardPage")loadDashboard();if(id==="bookingListPage")loadBookingList();if(id==="companyPage")loadCompanyProfileToForm();if(id==="masterDataPage")loadMasterDataPro("programs");if(id==="userPage")loadUsers();if(id==="permissionPage")loadPermissionMatrix();}
...
function applyAuth(){currentUser=JSON.parse(localStorage.getItem("user")||"null");document.getElementById("loginScreen").style.display=currentUser?"none":"flex";document.getElementById("userBox").innerHTML=currentUser?`<b>${currentUser.displayName}</b><br>Username: ${currentUser.username}<br>Role: ${currentUser.role}`:"";}
...
function parsePassengerText(){const lines=document.getElementById("passengerText").value.split("\\n").map(x=>x.trim()).filter(Boolean);const arr=[];for(const line of lines){const c=line.replace(/^\\d+[\\.)]\\s*/,"").trim();const m=c.match(/^(นาย|นาง|นางสาว|เด็กชาย|เด็กหญิง)\\s+(\\S+)\\s+(.+)$/);if(!m)return alert("Format ไม่ถูกต้อง: "+line);arr.push({title:m[1],firstName:m[2],lastName:m[3]});}passengers=arr.map((x,i)=>({...createPassenger(i),...x,isLeader:i===0}));document.getElementById("paxCount").value=passengers.length;renderPassengers();refreshSummary();}
...
function changeProgram(pi,id){const pr=master.programs.find(x=>x.program_id===id);passengers[pi].program={programId:pr.program_id,name:pr.program_name,qty:1,price:Number(pr.default_price),defaultPrice:Number(pr.default_price),priceReason:"ราคา Default"};renderPassengers();refreshSummary();}
...
async function saveBooking(){const b=buildBooking();if(!b.travelDate)return alert("กรุณาระบุวันเดินทาง");const dup=await API.duplicate(b);if(dup.duplicates?.length&&!confirm("พบข้อมูลอาจซ้ำ ต้องการ save ต่อไหม?"))return;await API.saveBooking(b);editingCode=b.bookingCode;alert("บันทึกแล้ว");await loadBookingList();}
