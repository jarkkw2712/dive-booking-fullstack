...
async function login(){try{const u=document.getElementById("loginUsername").value.trim(),p=document.getElementById("loginPassword").value;const r=await API.login(u,p);localStorage.setItem("token",r.token);localStorage.setItem("user",JSON.stringify(r.user));currentUser=r.user;applyAuth();if(r.user.mustChangePassword){showChangePassword(true,p);return}await initData();}catch(e){document.getElementById("loginError").innerText=e.message}}
function logout(){localStorage.clear();location.reload()}
function closeAuthModals(){["forgotPasswordModal","resetPasswordModal","changePasswordModal"].forEach(id=>document.getElementById(id)?.classList.add("hidden"));if(!currentUser)document.getElementById("loginScreen").style.display="flex"}
function showForgotPassword(){document.getElementById("loginScreen").style.display="none";document.getElementById("forgotPasswordModal").classList.remove("hidden")}
...
async function requestPasswordReset(){const root=document.getElementById("forgotMessage");root.innerText="กำลังส่ง...";try{const result=await API.forgotPassword(val("forgotEmail"));root.innerText=result.message}catch(error){root.innerText="ส่งคำขอไม่สำเร็จ กรุณาลองใหม่ภายหลัง"}}
async function submitPasswordReset(){const password=val("resetPassword"),confirmPassword=val("resetPasswordConfirm"),root=document.getElementById("resetMessage");if(password!==confirmPassword){root.innerText="รหัสผ่านไม่ตรงกัน";return}try{const token=new URLSearchParams(location.search).get("reset_token");const result=await API.resetPassword(token,password);history.replaceState({},"",location.pathname);root.innerText=result.message;setTimeout(()=>location.reload(),1500)}catch(error){root.innerText=error.message}}
...
function openUserEditor(){document.getElementById("userEditor").classList.remove("hidden");["userEditingId","userUsername","userEmail","userDisplayName","userTemporaryPassword"].forEach(id=>document.getElementById(id).value="");document.getElementById("userUsername").disabled=false;document.getElementById("userActive").value="true";loadRoles()}
async function editUser(id){const user=(_userRows||[]).find(row=>row.user_id===id);if(!user)return;await loadRoles();document.getElementById("userEditor").classList.remove("hidden");document.getElementById("userEditingId").value=id;document.getElementById("userUsername").value=user.username;document.getElementById("userUsername").disabled=true;document.getElementById("userEmail").value=user.email||"";document.getElementById("userDisplayName").value=user.display_name;document.getElementById("userRole").value=user.role_id;document.getElementById("userActive").value=String(user.active_flag);document.getElementById("userTemporaryPassword").value=""}
function closeUserEditor(){document.getElementById("userEditor").classList.add("hidden")}
async function saveUser(){const id=val("userEditingId"),payload={username:val("userUsername"),email:val("userEmail"),display_name:val("userDisplayName"),role_id:val("userRole"),active_flag:val("userActive")==="true",temporaryPassword:val("userTemporaryPassword")};if(id)await API.updateUser(id,payload);else await API.saveUser(payload);alert(id?"อัปเดต User แล้ว":"สร้าง User แล้ว; แจ้งรหัสชั่วคราวผ่านช่องทางที่ปลอดภัย");closeUserEditor();loadUsers()}
...
(async()=>{const resetToken=new URLSearchParams(location.search).get("reset_token");if(resetToken){document.getElementById("loginScreen").style.display="none";document.getElementById("resetPasswordModal").classList.remove("hidden");return}applyAuth();if(currentUser?.mustChangePassword){showChangePassword(true);return}if(currentUser)await initData();})();
